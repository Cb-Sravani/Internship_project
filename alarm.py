...
#app_image=Lable(frame_body,height=100,image=img)
...
def sound_alarm():
    mixer.music.load('scam.mp3')
    mixer.music.play()
    selected.set(0)
    rad2=Radiobutton(frame_body,font=('airal 10 bold'),value=2,text="deactivate",bg=co2,fg=bg_color,command=deactivate_alarm,variable=selected)
    rad2.place(x=140,y=95)
def alarm():
    while True:
        control=1
